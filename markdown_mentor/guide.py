...
software steps for you (build the pack, check it, export) and, for the thinking
...
Who does what:
- This command runs the build, check, and export steps.
...
def run_guide(source_dir: str | None = None) -> int:
    ...
    # Step 2: build
    _say("\nStep 2 of 9  -  Build the content pack  [software]")
    if _pause() != "s":
        try:
        ...
        _say("Skipped. Assuming the pack is at: " + str(pack_path))

    # Step 3: check
    _say("\nStep 3 of 9  -  Check the pack  [software]")
    if _pause() != "s" and pack_path.is_file():
        ...
        if report.risk_count:
            ...
            _say(f"If you want help fixing the pack, use the prompt:")
    ...
    _say("then paste your content pack:")
    ...
    _say("content pack, brief, pedagogy, material specifications, and list:")
    ...
    _say("Turn your finished Markdown into Word, slides, or web files.")
    try:
    ...
    _rule()
    _say("That is the whole workflow. For PDF, open a Word or web file and use")
    _say("'Save as PDF' in your own software. Read everything the AI produced.")
    _rule()
    return 0
